...
export default (logger: Logger) => async (ctx: Context, next: Next): Promise<void> => {
  try {
    await next()
    ...
    switch (status) {
      ...
      case httpStatusCode.UN_PROCESSABLE_ENTITY:
        logger.warn({ status, message, violations, method: ctx.method, id: ctx.state.id, user: ctx.user, url: ctx.url })
